<div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 md:p-6">
    ...
    <a href="#" class="shrink-0 md:order-1">
        <img class="h-20 w-20 dark:hidden" src="{{ env('APP_URL').'/storage/images/product/'.$productPhoto }}" alt="imac image" />
    </a>
    ...
    <div class="flex items-center justify-between md:order-3 md:justify-end">
        ...
        <input type="text" id="counter-input" data-input-counter class="w-10 shrink-0 border-0 bg-transparent text-center text-sm font-medium text-gray-900 focus:outline-none focus:ring-0 dark:text-white" placeholder="" value="{{ $orderItemQuantity }}" required />
        ...
        <p class="text-base font-bold text-gray-900 dark:text-white">Rp{{ $orderItemQuantity*$productPrice }}</p>
        </div>
    </div>
    <div class="w-full min-w-0 flex-1 space-y-4 md:order-2 md:max-w-md">
        <a href="#" class="line-clamp-2 text-base font-medium text-gray-900 hover:underline dark:text-white">{{ $productName }}</a>
